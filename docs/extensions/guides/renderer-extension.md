...
* The `install()` method installs Kubernetes resources using the `applyResources()` method, or by directly accessing the [Kubernetes API](../../api/lens-ide/README.md).
...
* The `uninstall()` method uninstalls Kubernetes resources using the [Kubernetes API](../../api/lens-ide/README.md).
...
* It implements `uninstall()` by utilizing the [Kubernetes API](../../api/lens-ide/README.md) which Lens provides to delete the `example-pod` applied by the `install()` method.

* It implements `updateStatus()` by using the [Kubernetes API](../../api/lens-ide/README.md) which Lens provides to determine whether the `example-pod` is installed, what version is associated with it, and whether it can be upgraded.
